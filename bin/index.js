...
const argv = await yargs(hideBin(process.argv))
    .command('run', 'Run RPCRelay')
    .command('config', 'Modify config file')
    .demandCommand(1)
    .argv;
...
switch (argv._[0]) {
    case 'run':
        ...
        app.all('/:chainId/*', async (req, res) => {
            const chainId = req.params.chainId;
            const path = req.path.replace(`/${chainId}`, '');
            if (!appData.chains.find((c) => c.chainId == chainId)) {
                res.status(404).send({
                    message: 'Chain not found'
                });
                return;
            }
            if (!appData.chains.find((c) => c.chainId == chainId)?.rpc) {
                res.status(404).send({
                    message: 'No valid RPCs found'
                });
                return;
            }
            const rpc = appData.chains.find((c) => c.chainId == chainId)?.rpc;
            // Shuffle RPCs
        ...
        while (doOver) {
            ...
            let prompt = await inquirer.prompt([
                ...
                },
                {
                    ...
                    choices: appData.chains.map((chain) => `${chain.name} (${chain.chainId})`)
                },
                {
                    name: 'actionChain',
                    message: 'What do you want to do?',
                ...
                },
                {
                    ...
                    choices: (answers) => {
                        const chain = appData.chains.find((chain) => chain.chainId == [...answers.chainEdit.match(regex)].pop().replace('(', '').replace(')', ''));
                        return chain.rpc;
                    }
                },
                {
            ...
            switch (prompt.action) {
                case 'Add Chain':
                    ...
                    break;
                case 'Edit Chain':
                    const chain = appData.chains.find((chain) => chain.chainId == [...prompt.chainEdit.match(regex)].pop().replace('(', '').replace(')', ''));
                    ...
                    break;
                case 'Remove Chain':
                    appData.chains.splice(appData.chains.indexOf(appData.chains.find((chain) => chain.chainId == [...prompt.chainRemove.match(regex)].pop().replace('(', '').replace(')', ''))), 1);
                    break;
                case 'Modify Server':
